...
const PopQuestion = ({ question, popTitle, popDescription, children }) => {
    ...
    return (
        <div className='container'>
            <span className='popup-question'>{question} <span className='pop-title' onClick={()=>setShow(true)}>{popTitle}</span>? <span className='info-icon' onClick={()=>setShow(true)}><InfoIcon/></span>
            {children}
            </span>
            <Modal show={show} closeModal={()=>setShow(false)}>
                <div className='modal-content'>
                    <span className='modal-title'><h4>{popTitle} <hr/></h4></span>
